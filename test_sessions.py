...
# --- Изменение 74: Тесты cleanup_expired и TTL ---


def test_cleanup_expired_removes_old_sessions(tmp_path):
    """cleanup_expired удаляет устаревшие сессии."""
    db = tmp_path / "test.db"
    sm = SessionManager(make_sqlite_config(db, ttl=0))
    sm.update_session("s1", "a")
    sm.update_session("s2", "b")
...
def test_get_session_skips_expired(tmp_path):
    """Сессия с истёкшим TTL -> get_session возвращает None."""
    db = tmp_path / "test.db"
    sm = SessionManager(make_sqlite_config(db, ttl=1))
    sm.update_session("s1", "музыка")
    time.sleep(1.1)
    entry = sm.get_session("s1")
    assert entry is None


def test_cleanup_expired_memory_mode():
    """cleanup_expired работает и для in-memory режима."""
    sm = SessionManager(make_config(ttl=0))
    sm.update_session("s1", "a")
    sm.update_session("s2", "b")
    time.sleep(0.01)
...
# --- Изменение 78: Тесты create_session ---
...
def test_create_session_sqlite_returns_unique_id(tmp_path):
    """SQLite: create_session возвращает уникальные ID."""
    db = tmp_path / "test.db"
    sm = SessionManager(make_sqlite_config(db))
    id1 = sm.create_session("термин1", "домен1")
    id2 = sm.create_session("термин2", "домен2")
    assert id1 != id2
    # Verify sessions exist
    assert sm.get_domain(id1) == "домен1"
    assert sm.get_domain(id2) == "домен2"
