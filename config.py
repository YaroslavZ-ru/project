...
class Config:
    ...
    def _validate(data: dict) -> None:
        ...
        # --- Изменение 73: api_workers ---
        aw = data.get("api_workers")
        if aw is not None:
            if not isinstance(aw, int) or aw <= 0:
                raise ValueError(f"api_workers должен быть положительным целым, получено: {aw!r}")

        # --- Изменение 77: search_cache_ttl_seconds ---
        sct = data.get("search_cache_ttl_seconds")
        if sct is not None:
            if not isinstance(sct, int) or sct < 0:
                raise ValueError(f"search_cache_ttl_seconds должен быть >= 0, получено: {sct!r}")
