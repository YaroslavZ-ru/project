...
# --- Изменение 73: Тест multi-worker ---


def test_api_starts_with_workers_flag():
    """Проверить что --workers передаётся в uvicorn (unit test)."""
    from unittest.mock import patch

    import scripts.run_api as run_api_mod

    with patch("scripts.run_api.uvicorn") as mock_uvicorn:
        # Simulate: run_api called with --workers 2
        import sys
        test_args = ["run_api.py", "--workers", "2", "--config", "configs/config.json"]
        with patch.object(sys, "argv", test_args):
            # We can't fully run __main__, but we can test the parser logic
            parser = run_api_mod.argparse.ArgumentParser()
            ...
            parsed = parser.parse_args(["--workers", "2"])
