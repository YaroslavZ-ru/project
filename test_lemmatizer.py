...
# --- M-6: lemmatize_word использует parse[0] ---


def test_lemmatize_word_uses_best_parse():
    """Убедиться что lemmatize_word возвращает правильную лемму."""
    lm = Lemmatizer()
    # "ключам" -> "ключ" (дательный падеж мн. числа)
    result = lm.lemmatize_word("ключам")
    assert result == "ключ"
