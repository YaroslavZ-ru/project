...
class SearchCache:
    """Кэш результатов поиска с TTL и LRU-эвикцией."""
...
class KnowledgeBase:
    def __init__(self, config, embedding_model=None, synonym_dict=None):
        ...
        self._faiss_rebuild_pending: bool = False
        ...
        self._matrix_lock: threading.RLock = threading.RLock()
        # --- Изменение 77: SearchCache с TTL ---
        self._search_cache = SearchCache(
            maxsize=getattr(config, "query_cache_size", 128),
    ...
    def save_concept(
        ...
        # --- Изменение 75: Инвалидация матрицы эмбеддингов ---
        self._invalidate_embeddings_matrix()
        # --- Изменение 77: Инвалидация кэша поиска ---
    ...
    def _maybe_rebuild_faiss(self):
        """Проверить необходимость пересборки FAISS и запустить в фоне."""
        ...
        threading.Thread(target=self._rebuild_faiss_async, daemon=True).start()

    def _rebuild_faiss_async(self):
        """Фоновая пересборка FAISS-индекса."""
        ...
        try:
            ...
            if N < 256:
                ...
                if N < min_train:
                    nlist = max(1, N // 39)
                if nlist < 1:
                    # Крайне мало данных для IVF — fallback на точный поиск
                    _faiss.normalize_L2(embeddings)
                    index = _faiss.IndexFlatIP(300)
                    index.add(embeddings)
                else:
                    quantizer = _faiss.IndexFlatIP(300)
                    index = _faiss.IndexIVFPQ(quantizer, 300, nlist, 64, 8)
                    index.train(embeddings)
                    index.add(embeddings)
                    index.nprobe = 32
            return index
        except ImportError:
            self.logger.warning("faiss не установлен. Пересборка невозможна.")
            return None
        except Exception as exc:
            self.logger.error("Ошибка построения FAISS: %s", exc)
            return None

    @property
    def _faiss_lock(self):
        """Лок для безопасной замены FAISS-индекса."""
        if not hasattr(self, "_faiss_rebuild_lock"):
            self._faiss_rebuild_lock = threading.Lock()
        return self._faiss_rebuild_lock

    # --- Изменение 75: Оптимизация матрицы эмбеддингов ---

    def _ensure_embeddings_matrix(self):
        """Ленивая инициализация матрицы эмбеддингов для линейного поиска."""
    ...
    def _invalidate_embeddings_matrix(self):
        """Сбросить кэш матрицы эмбеддингов при изменении данных."""
        with self._matrix_lock:
    ...
    def search_similar_concepts(self, query_vector, min_confidence=None, max_candidates=None, domain_filter=None):
        ...
        if self._config.use_faiss and len(concepts) > self._config.faiss_threshold:
            ...
            if self._faiss_index is not None:
                ...
                results = self._linear_search_with_matrix(
                    query_vector, min_confidence, max_candidates, domain_filter
                )
        else:
            results = self._linear_search_with_matrix(
                query_vector, min_confidence, max_candidates, domain_filter
        ...
        if len(results) < 3 and min_confidence > 0.2:
            ...
            results = self._linear_search_with_matrix(query_vector, 0.2, max_candidates, domain_filter)
        ...
        # --- Изменение 77: Сохранение в кэш ---
        self._search_cache.put(cache_key, results)
        return results

    def _linear_search_with_matrix(
        self,
        query_vector: np.ndarray,
        min_confidence: float,
        max_candidates: int,
        ...
        """Линейный поиск с использованием кэшированной матрицы эмбеддингов (изм. 75)."""
        ...
        if domain_filter:
            concepts = self.get_all_concepts(use_cache=True)
            domain_ids = {c["id"] for c in concepts if c.get("domain") == domain_filter}
            mask = np.array([i in domain_ids for i in ids])
        ...
        concepts_by_id = {c["id"]: c for c in self.get_all_concepts(use_cache=True)}
        results = []
        for idx in top_indices:
            if sims[idx] < min_confidence:
                break
            cid = ids[idx]
