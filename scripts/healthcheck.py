...
def check_config(config_path: Path) -> CheckResult:
    ...
    try:
        ...
        return CheckResult("config.json", "OK",
...
def check_database(config) -> CheckResult:
    ...
    try:
        ...
        conn.close()
        if n == 0:
            return CheckResult("База знаний (SQLite)", "WARN",
                              "БД пустая. Запустите scripts/seed_data")
...
def check_fasttext(config) -> CheckResult:
    ...
    if not model_path.exists():
        return CheckResult("FastText модель", "WARN",
    ...
    try:
        ...
        if not np.all(vec == 0):
            return CheckResult("FastText модель", "OK",
                              f"Загружена, dim={wrapper.get_dimension()}")
...
def check_fallback_embeddings(config) -> CheckResult:
    ...
    if not fb_path_str:
        return CheckResult("fallback_embeddings", "WARN",
                          "Не задан. Запустите scripts/build_fallback")
...
def check_synonyms(config) -> CheckResult:
    ...
    try:
        ...
        if len(data) == 0:
            return CheckResult("synonyms.json", "WARN",
...
def check_domain_templates(config) -> CheckResult:
    ...
    for attr, label in [("domain_templates_path", "domain_templates.json"),
...
def check_faiss(config) -> CheckResult:
    ...
    if faiss_path_str and not Path(faiss_path_str).exists():
        return CheckResult("FAISS", "WARN", f"FAISS индекс не найден: {faiss_path_str}. Запустите build_faiss")
...
def check_prometheus(config) -> CheckResult:
    ...
    try:
        ...
        return CheckResult("Prometheus", "WARN", "use_metrics=true но prometheus_client не установлен")
...
def check_logs_dir() -> CheckResult:
    ...
    if not logs_dir.exists():
        return CheckResult("Директория logs/", "WARN",
