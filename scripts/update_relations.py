...
def validate_relation(rel: dict) -> list[str]:
    ...
    if rel.get("relation_type") not in VALID_RELATION_TYPES:
        errors.append(
            f"недопустимый relation_type: {rel.get('relation_type')} допустимые={VALID_RELATION_TYPES}"
        )
    ...
    try:
        ...
        for rel in data:
            ...
            exists_src = conn.execute(
                "SELECT 1 FROM concepts WHERE id=?", ([src])
            ).fetchone()
            exists_tgt = conn.execute(
                "SELECT 1 FROM concepts WHERE id=?", ([tgt])
            ).fetchone()
        ...
        conn.close()
        logger.info(
            "Отношения: inserted=%d skipped=%d errors=%d", inserted, skipped, errors
        )
